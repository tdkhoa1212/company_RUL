...
def fully_concatenate(hidden_out_1D, hidden_out_2D, hidden_out_extracted, training=None, fully=None):
    if fully:  
      ...
      hidden_out_1D = tf.keras.layers.Dense(hidden_out_extracted.shape[-1],   activation='relu',
      ...
      hidden_out_2D = tf.keras.layers.Dense(hidden_out_extracted.shape[-1],   activation='relu',
                                          kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                                          bias_regularizer=regularizers.l2(1e-4),
      ...
      all_ = hidden_out_1D + hidden_out_2D + hidden_out_extracted
      all_ = Dropout(0.2)(all_, training=training)
      return all_
...
#   out_2D = base_model_2D(input_2D, training=training)
  ...
  merged_value_0 = fully_concatenate(rul_hidden_out_1D, rul_hidden_out_2D, rul_hidden_out_extracted, training=training, fully=True)
