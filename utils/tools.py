...
def extract_feature_image(data_2c, type_data, feature_name, opt):
    WAVELET_TYPE = 'morl'
    DATA_POINTS_PER_FILE=opt.input_shape
...
def convert_to_image(name_bearing, opt, type_data):
    ...
    for data_2c in data_all:
        coef_h = extract_feature_image(data_2c, type_data, 'Horizontal_vibration_signals', opt)
        coef_v = extract_feature_image(data_2c, type_data, 'Vertical_vibration_signals', opt)
...
def get_pre_data(file, opt, type_data, fold=False):
    if fold:
      file = file[:, :2]
    data = {'x': [], 'y': []}
    ...
    ############### process data #############
    coef_h = extract_feature_image(data_all, type_data, 'Horizontal_vibration_signals', opt)
    coef_v = extract_feature_image(data_all, type_data, 'Vertical_vibration_signals', opt)
    x_ = np.concatenate((coef_h, coef_v), axis=-1)
    data['x'] = np.expand_dims(x_, axis=0)
